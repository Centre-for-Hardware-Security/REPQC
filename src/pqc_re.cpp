...
int main(int argc, char** argv) { 
	...
	// this is strategy 2, it works by eliminating groups based on z-score ordering. it works well if the keccak state flip-flops are either at the top of the list
	// or they look very different from other regs
	// this is mentioned in the paper as REPQC(g)
	#ifdef STRAT_G
		printGroups();
	...
	// this is strat 3, works for masked circuits. it is mentioned in the paper as REPQC(i)
	#ifdef STRAT_I
		printGroups();
		eliminateGroupsByRegisterPath(candidates);
		eliminateMembersWithoutHits();
	...
	cout << "REPQC took " << (float)duration.count()/1000000 << " seconds" << endl;
	return 0; 
}
...
void printGroups() {
	...
	logfile << "generated by REPQC" << endl;
...
void printShrinkGroups(float tolerance) {
	ofstream logfile;
	logfile.open ("../work/groups.log");
	logfile << "generated by REPQC. src code available from https://github.com/Centre-for-Hardware-Security/re_pqc \n";
	logfile << "the structure is as follows:" << endl;
	logfile << "groups are separated by -------- group ID -------- lines" << endl;
...
void eliminateGroupsByRegisterPath(int candidates) {
	int regcount = 0;
	int reg_passed_filter = 0; // this is the variable reported in Table 2 of the paper
	int considered = 0;

	ofstream logfile1, logfile2;
	logfile1.open ("../work/candidates.log");
	logfile2.open ("../work/candidates_under_1.log");
	...
	for (auto& it1 : myregs_zordered) {
		...
		//if (myregs[it1].score >= 1.0) break; // TODO: this does not always work well, but it does eliminate any reg that is obviously control oriented
		// for now it is not being used, only reported on the logs. it caused issues with kali design from design compiler and saber_v1. very interesting. it would speed up the execution time...
		// an alternative would be to find the dominant z-value and eliminate those above it
		...
		logfile1 << "analyzing " << it1 << " as target. fanin " << deps.size()  << ", fanout " << fanout[it1].size() << ", z-score  " << myregs[it1].score << endl;
		if (myregs[it1].score < 1.0) {
			logfile2 << "analyzing " << it1 << " as target. fanin " << deps.size()  << ", fanout " << fanout[it1].size() << ", z-score  " << myregs[it1].score << endl;
		}
	...
	cout << reg_passed_filter << " passed fanin/fanout filter, ";
